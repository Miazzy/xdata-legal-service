  ...
  methods: {
      ...
      async queryInfo() {
        try {
          ...
          this.data = await this.handleList(tableName , '执行阶段', userinfo, '' , 0 , 10000);
        } catch (error) {
          console.log(error);
        }
      },
      ...
      // 查询不同状态的领用数据
      async handleList(tableName , stage = '执行阶段', userinfo, searchSql , page = 0 , size = 10000){
      ...
      async execSearch(value){
        ...
        this.data = await this.handleList(tableName , '执行阶段', userinfo, searchSql , 0 , 10000);
      },
