...
export default {
  ...
  data() {
    return {
      ...
      data:[],
      rowSelection:[],
      breadcrumb:[{icon:'home',text:'首页',path:'/legal/workspace'},{icon:'user',text:'案件管控',path:'/legal/workspace'},{icon:'form',text:'二审管理',path:''}],
  ...
  methods: {
      ...
      async queryInfo() {
        try {
          ...
          this.data = await this.handleList(tableName , '二审阶段', userinfo, '' , 0 , 10000);
        } catch (error) {
          console.log(error);
        }
      },
      ...
      // 查询不同状态的领用数据
      async handleList(tableName , stage = '二审阶段', userinfo, searchSql , page = 0 , size = 10000){
      ...
      async execSearch(value){
        ...
        this.data = await this.handleList(tableName , '二审阶段', userinfo, searchSql , 0 , 10000);
      },
